...
(except spaces, hyphens, and underscores).
"""
import argparse
import logging
import re
...
def rename_mp4_clean(dir_path: Path, dry_run: bool = False):
    ...
    spaces, hyphens, and underscores), and rename them accordingly.
    ...
    for path in dir_path.rglob('*'):
        if path.is_file() and path.suffix.lower() == '.mp4':
            ...
            # Lowercase and remove special characters (allow a-z, 0-9, spaces, hyphens, and underscores)
            new_stem = re.sub(r'[^a-z0-9 _-]', '', stem.lower())
            # Optionally, strip leading/trailing spaces
            new_stem = new_stem.strip()
            new_name = f"{new_stem}{suffix}"
            if path.name != new_name:
                new_path = path.with_name(new_name)
                logging.info(f"Renaming: '{path.name}' -> '{new_name}'")
                if not dry_run:
                    try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Recursively rename .mp4 files to lowercase filenames and remove special characters (spaces, hyphens, and underscores are allowed)."
    )
    parser.add_argument(
        'directory',
        nargs='?', 
